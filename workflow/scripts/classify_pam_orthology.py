...
class SyntenyBlock:
    ...
    def project_ref_to_query(self, ref_pos: int) -> float:
        """
        Project a reference coordinate into query coordinates by fractional
        position through the synteny block.

        For inverted blocks, the left side of the reference maps to the right
        side of the query interval.
        """
...
def _get_first_attr(obj: object, names: Iterable[str]) -> object:
    for name in names:
        if hasattr(obj, name):
            return getattr(obj, name)
    raise AttributeError(
        f"Could not find any of these attributes on {type(obj).__name__}: "
        f"{', '.join(names)}"
    )
...
def _block_from_svmu2_object(block: object, fallback_id: str) -> SyntenyBlock:
    """
    Convert an svmu2 block-like object into a SyntenyBlock.

    The attribute list accepts both legacy delta-parser names and newer synteny
    block geometry names such as left_most/right_most/top/bottom.
    """
    ref_chr = str(_get_first_attr(block, ["reference", "ref_chr", "ref", "reference_name"]))
    query_chr = str(_get_first_attr(block, ["query", "query_chr", "qry", "query_name"]))

    ref_start = _coerce_int(_get_first_attr(block, ["reference_start", "ref_start", "r_start", "left_most"]))
    ref_end = _coerce_int(_get_first_attr(block, ["reference_end", "ref_end", "r_end", "right_most"]))

    # query_start/query_end preserve orientation when available. top/bottom are
    # accepted as a fallback but are usually normalized coordinates, so an
    # inverted block may lose orientation if only top/bottom are available.
    query_start = _coerce_int(_get_first_attr(block, ["query_start", "qry_start", "q_start", "top"]))
    query_end = _coerce_int(_get_first_attr(block, ["query_end", "qry_end", "q_end", "bottom"]))

    block_id = str(
        getattr(
            block,
            "block_id",
            getattr(block, "index", getattr(block, "id", fallback_id)),
        )
    )
...
def build_synteny_blocks_from_delta(delta_file: str | Path) -> list[SyntenyBlock]:
    ...
    for aln_key, aln in primary.items():
        for i, block in enumerate(_iter_alignment_blocks(aln)):
            fallback_id = f"{getattr(aln, 'reference', aln_key)}__{getattr(aln, 'query', 'query')}__{i}"
            b = _block_from_svmu2_object(block, fallback_id=fallback_id)
